		...
		<div class="span12">      		
      		...
      		<div class="widget stacked ">
      			...
      			<div class="widget-header">
      				...
      				<h3>Import photos from googleplus</h3>
  				</div> <!-- /widget-header -->
				...
				<div class="widget-content">
						...
						<div style="width:100%;margin-bottom: 20px;">
							<div style=" text-align: left; display: inline-block; width: 50%;">
								<span style="font-size: 25px;">Photos from Google+</span><br>
					...
					<div id="container" style="display:none;width:100%">
						...
						<? if($pics){?>
							<?foreach($pics['pics'] as $album){?>
								<div class="grid" id="<?=$i?>">
									<!--<div class="photohover" id="hover<?//=$i?>"><img src="<?//=base_url()?>img/plus.png"><b>&nbsp; Add to PhotoStream</b></div>-->
									<div class="imgholder">
										<label class="ps-label" for="google<?=$i?>">
										<img id="img<?=$i?>" src="<?=$album['content'][0]?>" alt="<?=$album['title'];?>" />
										</label>
									</div>
									<div class="meta"></div>
									<input id="google<?=$i?>" class="check" name="checkupload" id="checkupload" type="checkbox" value="<?=$album['content'][0]?>" />
								</div>
								<? $i++; ?>
							<? } ?>
						<? } ?>
...
<script type="text/javascript">
	...
	$('#addstream').click(function(){
		...
		if(selected_pics.length <= 0){
				...
				$.post('/stream/save_google_pics',{selected_pics:selected_pics,counter:counter},function(data){
