			...
			<div class="span12" style="padding: 4px;">
				...
				<?endif;?>
					<span>
						...
						<p style="text-align: left; color: #000; font-size: 12px;">
							<div class="controls">
								<textarea id="comment_<?=$data->comment_id?>" class="input-block-level" rows="2" placeholder="Write a comment..." style="font-size: 11px;"><?=$data->comment?></textarea>
								<!--<input type="hidden" id="commenttext_<?//=$data->comment_id?>" value="<?//=$data->comment?>">-->
								<button id="editcom_<?=$data->comment_id?>" class="btn btn-success btn-small editcom">Edit</button>
								<button id="cancelcom_<?=$data->comment_id?>" class="btn btn-danger btn-small cancelcom">Cancel</button>
							</div>
						</p>
					</span>
					<div class="row-fluid">
...
$(function(){
		...
		$('.edit').click(function(){
			...
			$.post(base_url+'photo/editcomment',{id:id},function(data){
		...
		$('.editcom').click(function(){
			...
			var str = $(this).attr('id');
			var id = str.replace('editcom_',"");
			var base_url = $('#base_url').val();
			var photoID = $('#photoID').val();
			//var text = $('#commenttext_'+id).val();
			...
			if(text == ""){
				...
				$.post(base_url+'photo/savecommentedit',{id:id,text:text},function(data){
