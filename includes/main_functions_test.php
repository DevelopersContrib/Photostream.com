...
function getReferral(){
  ...
  function getServiceType($id=''){
    ...
    function SaveLeads($email,$domain,$user_ip){
		...
		if($total<1){
			...
			$result = mysql_query("Insert into leads (email,datetime_created,domain_name,ip) Values

				('$email',NOW(),'$domain','$user_ip')");
	...
	function GetRandomDomains($domainName){
		$returnHTML = "";
		try {
			$host = "127.0.0.1";
			...
			while($row = $STH->fetch()) {
				...
				echo '<a href="http://'.$row['domain_name'].'">'.$row['domain_name'].'</a><br>';
			}
