...
class PhotoUsers extends CI_Model {
	...
	private $pk = "userid";
	private $date = 'date_signedup';		private $date_lead = 'datetime_created';		private $db_domaindi = null;		private $table_lead = "leads";		private $pk_lead = "id";			 function __construct() {      parent::__construct();      $this->db_domaindi = $this->load->database('domaindi', TRUE);   }
    ...
    return $returnValue;
	}		public function isAdmin($userid){				$returnValue = '';		/*if($userid == "343"){					return true;				}else{						return false;		}*/				$query = $this->db->query("SELECT * FROM `$this->table` where `userid` = '".$userid."'");		if ($query->num_rows() > 0){			foreach ($query->result() as $row)			 {			   $returnValue =  $row->admin;			 }		   }		   	  		if ($returnValue=="1"){		  return true;				}else {			return false;		}		}
  ...
  function update($id,$data){
   ...
   }      function updatedomaindi($id,$data){   		 $query = $this->db_domaindi->query("Select * from `$this->table_lead` where $this->pk_lead = '$id'"); 		if ($query->num_rows() > 0){	     $this->db_domaindi->where($this->pk_lead, $id);		 $this->db_domaindi->update($this->table_lead, $data);		 return $id;		} else {			if ($this->date_lead){			  $this->db_domaindi->set($this->date_lead, 'NOW()', FALSE);			}			$this->db_domaindi->insert($this->table_lead, $data);			return $this->db_domaindi->insert_id();		}				         }
