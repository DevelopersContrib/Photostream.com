...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/uploads/profile/',
            'upload_url' => $this->get_full_url().'/uploads/profile/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function handle_form_data($file, $index) {
		...
		$new_filename = time();
		 ...
		 switch (strtolower(substr(strrchr($file->name, '.'), 1))) {
            case 'jpg':
            case 'jpeg':
                $file->name = $new_filename.'.jpg';
                break;
            case 'gif':
               $file->name = $new_filename.'.gif';
                break;
            case 'png':
               $file->name = $new_filename.'.png';
                break;
            default:
                return false;
        }
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
					...
					//$file_path = $upload_dir.$file->name.".".$file->type;
