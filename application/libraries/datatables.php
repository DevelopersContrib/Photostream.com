  ...
  */
  class Datatables
  {
    ...
    */
	...
	added by sheina dec 28, 2012
	*/
	public function order_by($col,$cond){
		$this->order_by = $col;
			$this->ci->db->order_by($col,$cond);
		return $this;
	}
    ...
    */
    protected function get_filtering()
    {
     ...
     //passed parameters
      $team_id = $this->ci->db->escape($this->ci->input->get('team_id'));
      $domain_id = $this->ci->db->escape($this->ci->input->get('domain_id'));
      $receiver_id = $this->ci->db->escape($this->ci->input->get('receiver_id'));
      $sender_id = $this->ci->db->escape($this->ci->input->get('sender_id'));
      $team_tool_id = $this->ci->db->escape($this->ci->input->get('team_tool_id'));
      $keyword = $this->ci->db->escape($this->ci->input->get('keyword'));
      
	  $lead_domains = $this->ci->db->escape($this->ci->input->get('lead_domains'));
	  $edu_domains = $this->ci->db->escape($this->ci->input->get('edu_domains'));
	  
      if ($team_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND team_member.team_id = $team_id";
      	}else {
      		$sWhere = " ($sWhere) team_member.team_id = $team_id";
      	}
      }

      if ($domain_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND contact.domain_id = $domain_id";
      	}else {
      		$sWhere = " ($sWhere) contact.domain_id = $domain_id";
      	}
      }
      
    if ($receiver_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND messages.receiver_id = $receiver_id";
      	}else {
      		$sWhere = " ($sWhere) messages.receiver_id = $receiver_id";
      	}
      }
      
    if ($sender_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND messages.sender_id = $sender_id";
      	}else {
      		$sWhere = " ($sWhere) messages.sender_id = $sender_id";
      	}
      }
      
    if ($team_tool_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND tool_team.team_id = $team_tool_id";
      	}else {
      		$sWhere = " ($sWhere) tool_team.team_id = $team_tool_id";
      	}
      }
     
	 if ($lead_domains!=""){
		$lead_domains = preg_replace("/'/","",$lead_domains);
      	if ($sWhere != ''){
			$OR = '';
			$lead_domains = explode(',',$lead_domains);
			
			foreach($lead_domains as $lead_domain){
				$OR .=!empty($OR)?" OR ":"";
				$OR .= " domain_name = '$lead_domain' ";
			}
			$sWhere = !empty($OR)? " ($sWhere) AND ($OR) ":" ($sWhere) ";
      	}else {
      		$sWhere = " ($sWhere) domain_name = '$lead_domains' ";
      	}
		
      }
      
if ($edu_domains!=""){
		$edu_domains = preg_replace("/'/","",$edu_domains);
      	if ($sWhere != ''){
			$OR = '';
			$edu_domains = explode(',',$edu_domains);
			
			foreach($edu_domains as $edu_domain){
				$OR .=!empty($OR)?" OR ":"";
				$OR .= " domain = '$edu_domain' ";
			}
			$sWhere = !empty($OR)? " ($sWhere) AND ($OR) ":" ($sWhere) ";
      	}else {
      		$sWhere = " ($sWhere) domain = '$edu_domains' ";
      	}
