				...
				<div class="widget-content">
						...
						<div style="width:100%;margin-bottom: 20px;">
							<div style=" text-align: left; display: inline-block; width: 50%;">
								<span style="font-size: 25px;">Photos from Facebook </span><br>
								<span> (Select a photo to add to PhotoStream)</span>
							</div>
							<div style=" text-align: right; display: inline-block; width: 49%;">
							<form id="add_more_form" action="#" method="POST" style="margin:0">
								...
								<a href="<?=$this->facebook->getLoginUrl(array('scope' => 'user_photos','redirect_uri'=>'http://www.photostream.com/stream/facebook_photos'))?>" class="btn btn-tertiary">
										&nbsp;<i class="icon-long-arrow-left"></i>&nbsp;Select Albums
								</a>
						...
						<? if($photos){?>
							<?foreach($photos['data'] as $val2){?>
								<div class="grid" id="<?=$i?>">
									...
									<div class="imgholder">
										<label class="ps-label" for="fb<?=$i?>">
										<img id="img<?=$i?>" src="<?=$val2['source']?>" alt="<? echo "facebook ".$x;?>" />
										</label>
									</div>
									<div class="meta"></div>
									<input id="fb<?=$i?>" class="check" name="checkupload" id="checkupload" type="checkbox" value="<?=$val2['source']?>" />
								</div>
								<? $i++; ?>
							<? } ?>
						<? } ?>
...
<script type="text/javascript">
	...
	$('#addstream').click(function(){
		...
		if(selected_pics.length <= 0){
				...
				$.post('/stream/save_facebook_pics',{selected_pics:selected_pics,counter:counter},function(data){
