				...
				<div class="widget-content">
						...
						<div style="width:100%;margin-bottom: 20px;">
							<div style=" text-align: left; display: inline-block; width: 50%;">
								<span style="font-size: 25px;">Photos from Twitter </span><br>
							...
							<form id="add_more_form" action="/stream/upload" method="POST" style="margin:0">
								...
								<!--<a href="<?//=base_url()?>stream/album/<?//=url_title($this->photostream->getinfobyid('title',$id))?>/<?//=$this->session->userdata('stream_id')?>">View this Stream</a>-->
							</div>
						</div>
						...
						<? if($tweet){?>
							<?foreach($tweet as $r){?>
							    <? if(isset($r->entities->media)){?>
								      <? foreach($r->entities->media AS $e){?>
										<? if(isset($e->media_url)){?>
											  <div class="grid" id="<?=$i?>">
												  <!--<div class="photohover" id="hover<?//=$i?>"><img src="<?//=base_url()?>img/plus.png"><b>&nbsp; Add to PhotoStream</b></div>-->
												  <div class="imgholder">
													<label class="ps-label" for="tweet<?=$i?>">
													  <img id="img<?=$i?>" src="<?=$e->media_url?>" alt="<? echo "twitter ".$x;?>" />
													</label>
												  </div>
												  <div class="meta"></div>
												  <input id="tweet<?=$i?>" class="check" name="checkupload" id="checkupload" type="checkbox" value="<?=$e->media_url?>" />
											  </div>
											  <? $i++; ?>
											  <?}?>
										<?}?>
...
<script type="text/javascript">
	...
	$('#addstream').click(function(){
		...
		if(selected_pics.length <= 0){
				...
				$.post('/stream/save_twitter_pics',{selected_pics:selected_pics,counter:counter},function(data){
