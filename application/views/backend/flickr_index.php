					...
					<? $id = $this->session->userdata('stream_id'); ?>
						<div style="width:100%;margin-bottom: 20px;">
							<div style=" text-align: left; display: inline-block; width: 50%;">
								<span style="font-size: 25px;">Photos from Flickr </span><br>
						...
						<? if(isset($photos)){?>
							<? foreach($photos as $photo){?>
								<div class="grid" id="<?=$photo['id']?>">
									...
									<div class="imgholder">
										<label class="ps-label" for="flickr<?=$photo['id']?>">
										<img id="img<?=$photo['id']?>" src="<?=$photo['url']?>" alt="<? echo "flickr ".$x;?>" />
										</label>
									</div>
									<div class="meta"></div>
									<input id="flickr<?=$photo['id']?>" class="check" name="checkupload" id="checkupload" type="checkbox" value="<?=$photo['url']?>" />
								</div>
								<? $i++; ?>
							<? } ?>
						<? } ?>
...
<script type="text/javascript">
	...
	$('#addstream').click(function(){
		...
		if(selected_pics.length <= 0){
				...
				$.post('/stream/save_flickr_pics',{selected_pics:selected_pics,counter:counter},function(data){
